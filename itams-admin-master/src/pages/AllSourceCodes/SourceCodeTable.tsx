...
import { deleteSourceCode, getAllSourceCodes } from '../../api/sourceCode';
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof SourceCode) => (event: React.MouseEvent<unknown>) => {
...
export default function SourceCodeTable() {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof SourceCode>('id');
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const data = await getAllSourceCodes();
      setRows(data);
    } catch (err) {
      console.log(err);
    }
