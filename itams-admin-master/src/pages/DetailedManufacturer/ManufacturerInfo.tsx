...
export default function ManufacturerInfo(props: any) {
  const { manufacturerId } = props;
  const { getNotifications } = useAuthContext();
  const [rows, setRows] = React.useState<Manufacturer>();
  ...
  const getData = async () => {
    try {
      const manufacturer: Manufacturer = await getManufacturerById(
        manufacturerId,
      );
      console.log(manufacturer);
      setRows(manufacturer);
    } catch (err) {
      console.log(err);
    }
  };
